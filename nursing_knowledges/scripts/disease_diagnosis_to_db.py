...
def run():
    ...
    for row in range(df.shape[0]): 
        ...
        # 중분류가 DB에 없다면 새로 만들기
            ...
            medium_category_obj = DiseaseMediumCategory.objects.create(name=medium_category_data, disease_large_category=large_category_obj)
        ...
        if small_category_data is not NaN:
            ...
            # 소분류가 DB에 없다면 새로 만들기
                ...
                small_category_obj = DiseaseSmallCategory.objects.create(name=small_category_data, disease_medium_category=medium_category_obj)
        ...
        if diagnosis_data is not NaN:
            ...
            if small_category_data is not NaN:
                diagnosis_to_small = DiagnosisToOther(diagnosis=diagnosis_obj, disease_small_category=small_category_obj)
                diagnosis_to_small.save()
            # 소분류가 빈값이라면 진단 <--> 중분류 연결
            else:
                diagnosis_to_medium = DiagnosisToOther(diagnosis=diagnosis_obj, disease_medium_category=medium_category_obj)
